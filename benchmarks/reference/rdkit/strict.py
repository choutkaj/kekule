...
def graph(mol, legacy):
    atoms = []
    for atom in mol.GetAtoms():
        value = legacy.atom_json(atom)
...
def records(source, legacy):
    if source.suffix.lower() in ('.smi', '.smiles', '.txt'):
        ...
        for line in source.read_text().splitlines():
            ...
            mol = Chem.MolFromSmiles(parts[0], params)
            if mol is None:
                raise ValueError('RDKit SMILES parse failed')
            mol.UpdatePropertyCache(strict=False)
            Chem.SetBondStereoFromDirections(mol)
            values.append((parts[1] if len(parts) == 2 else '', mol, []))
        return values
    blocks = legacy.read_sdf_blocks(source) if source.suffix.lower() == '.sdf' else [source.read_text()]
    result = []
    for block in blocks:
...
def evaluate(feature, source, legacy):
    outputs = []
    for index, (title, mol, fields) in enumerate(records(source, legacy)):
...
def writer_value(feature, source, legacy):
    ...
    value = evaluate('molecule', source, legacy)
...
def read_written(feature, value, legacy, memory_input):
    ...
    for item in value['written']:
        ...
        outputs.extend(writer_value(feature, memory_input(item['path'], item['text']), legacy)['records'])
