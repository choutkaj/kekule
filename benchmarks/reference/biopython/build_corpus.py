#!/usr/bin/env python3
"""Build deterministic PDB benchmark corpora from pinned or reselected membership."""
...
BASE_SELECTION_ID = "pdb-v1"
EXPANSION_SELECTION_ID = "pdb-v2"
CATEGORIES = (
    "multi-model",
...
def main() -> int:
    ...
    parser.add_argument(
        "--reselect",
        nargs=2,
        metavar=("BASE_SELECTION_ID", "EXPANSION_SELECTION_ID"),
        help="Explicitly select new PDB membership under two new corpus version IDs.",
    )
    parser.add_argument(
        "--repo-root",
        type=Path,
    ...
    if args.reselect:
        base_selection_id, expansion_selection_id = args.reselect
        if (
            base_selection_id == BASE_SELECTION_ID
            or expansion_selection_id == EXPANSION_SELECTION_ID
        ):
            raise SystemExit("--reselect must name new corpus version IDs")
        (
            small_entries,
            base_entries,
            target_entries,
            base_source,
            target_source,
        ) = reselect_entries(base_selection_id, expansion_selection_id)
    else:
        base_entries, base_source, base_selection_id = load_locked_entries(base_root)
        small_entries, _, small_selection_id = load_locked_entries(small_root)
        target_entries, target_source, expansion_selection_id = load_locked_entries(
            target_root
        )
        if (
            small_selection_id != base_selection_id
            or entry_keys(small_entries) != entry_keys(base_entries[: len(small_entries)])
            or entry_keys(base_entries) != entry_keys(target_entries[: len(base_entries)])
        ):
            raise SystemExit("checked-in PDB source locks violate nested-prefix membership")
    build_tier(
        small_root,
        small_entries,
        base_source,
    ...
    build_tier(
        ...
        target_entries,
        target_source,
        selection_id=expansion_selection_id,
        include_mmcif_manifest=True,
    )
...
def reselect_entries(
    base_selection_id: str, expansion_selection_id: str
    ...
    target_source = (
        f"{base_source}; base selection ID {base_selection_id}"
    )
    return small_entries, base_entries, target_entries, base_source, target_source


def load_locked_entries(root: Path) -> tuple[list[dict], str, str]:
    lock_path = root / "sources.lock.json"
    lock = json.loads(lock_path.read_text(encoding="utf-8"))
    locked = [(entry["id"], entry["category"]) for entry in lock["entries"]]
    with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
        fetched = list(executor.map(fetch_candidate, (item[0] for item in locked)))
    entries = []
    for (pdb_id, category), item in zip(locked, fetched):
        if item is None:
            raise SystemExit(f"locked PDB entry {pdb_id} could not be reconstructed")
        if item["id"] != pdb_id or item["category"] != category:
            raise SystemExit(
                f"locked PDB entry {pdb_id} changed category from {category} to {item['category']}"
            )
        entries.append(item)
    return entries, lock["source"], lock["selection_id"]


def entry_keys(entries: list[dict]) -> list[tuple[str, str]]:
    return [(item["id"], item["category"]) for item in entries]
        ...
        for category in CATEGORIES:
            ...
            for start in range(0, len(candidates), 80):
                ...
                for result in executor.map(fetch_candidate, batch):
                    ...
                    used.add(result["id"])
                print(
                    f"selection_id={selection_id} category={category} examined={examined} "
            ...
            if len(selected[category]) < target_per_category:
                raise SystemExit(
                    ...
                    f"for selection ID {selection_id}"
                )
...
def fetch_candidate(pdb_id: str) -> dict | None:
    ...
    temp = Path.cwd() / "target" / "benchmark-pdb" / f"{pdb_id}.cif"
    if temp.exists():
...
def fetch(url: str, *, timeout: int = 45, attempts: int = 5) -> bytes:
    request = urllib.request.Request(url, headers={"User-Agent": "benchmark-reference/1"})
...
def post_json(url: str, payload: dict) -> dict:
    request = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "benchmark-reference/1",
        },
    )
    ...
    *,
    ...
    for index, line in enumerate(descriptor_lines):
        if line.startswith("selection_id = "):
            descriptor_lines[index] = f'selection_id = "{selection_id}"'
            descriptor.write_text(
                "\n".join(descriptor_lines) + "\n", encoding="utf-8"
            )
            break
    else:
        raise SystemExit(f"{descriptor} has no selection_id")
...
def generate_golden(repo: Path, corpus: str) -> None:
    output_dir = repo / "benchmarks" / "corpora" / corpus / "golden" / FEATURE_ID
    if output_dir.exists():
        shutil.rmtree(output_dir)
    script = repo / "benchmarks" / "reference" / "biopython" / "run_feature.py"
    subprocess.run(
